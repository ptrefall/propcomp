...
class CL_API_GL1 LightSource_GL1
{
//!Construction
public:
...
public:
	...
	void set_ambient_intensity(const Colorf &color);

	/// \brief  Sets the diffuse (spread over wide area) intensity of the light.
	void set_diffuse_intensity(const Colorf &color);

	/// \brief  Sets the specular (mirror reflective) intensity of the light.
	void set_specular_intensity(const Colorf &color);
	...
	void set_position(const Vec4f &position);

	/// \brief  Sets the light spot direction.
	void set_spot_direction(const Vec3f &spot_direction);

//!Implementation
private:
	std::shared_ptr<LightSource_GL1_Impl> impl;
};
