...
class GL1WindowDescription_Impl;
...
///    GL1Window window(desc);
///    </pre> 
/// \xmlonly !group=GL1/Display! !header=gl1.h! \endxmlonly
class CL_API_GL1 GL1WindowDescription : public DisplayWindowDescription
{
/// \name Construction
/// \{
public:
	/// \brief Constructs a window description with default values.
	GL1WindowDescription();

	/// \brief Constructs a GL1WindowDescription
	///
	/// \param desc = Display Window Description
	GL1WindowDescription(const DisplayWindowDescription &desc);
	GL1WindowDescription &operator=(DisplayWindowDescription &desc);

	virtual ~GL1WindowDescription();
...
/// \{
private:
	std::shared_ptr<GL1WindowDescription_Impl> impl_gl;
/// \}
};
