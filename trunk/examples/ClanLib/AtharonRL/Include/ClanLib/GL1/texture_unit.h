...
///
/// \xmlonly !group=GL1/Display! !header=gl1.h! \endxmlonly
class CL_API_GL1 TextureUnit_GL1
{
//!Construction
public:
...
public:
	...
	TextureUnitMode get_mode() const;
	...
	TextureUnitRGBMode get_combine_rgb_mode() const;
...
public:
	...
	void set_mode(TextureUnitMode mode);
	...
	void set_combine_mode(TextureUnitRGBMode rgb_mode, TextureUnitAlphaMode alpha_mode);
	...
	void set_combine_rgb_arg(int arg, TextureUnitSource src_rgb, TextureUnitRGBOperand operand_rgb);

	/// \brief  Sets an arguement for the alpha combine texture function.
	void set_combine_alpha_arg(int arg, TextureUnitSource src_alpha, TextureUnitAlphaOperand operand_alpha);
...
//!Implementation
private:
	std::shared_ptr<TextureUnit_GL1_Impl> impl;
};
